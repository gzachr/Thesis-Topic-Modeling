...
data_lda = load_json("lda_results.json")
...
# ---------------------------------
    ...
    if "selected_video" not in st.session_state:
        st.session_state.selected_video = list(data_lda["videos"].keys())[0]
    ...
    if tab == "Model Summary":
        ...
        with col1:
            st.metric("Coherence (c_v)", f"{data_lda['model_metrics']['coherence_c_v']:.3f}")
            st.metric("Coherence (u_mass)", f"{data_lda['model_metrics']['coherence_u_mass']:.3f}")
            st.metric("Jaccard Similarity", f"{data_lda['model_metrics']['jaccard_similarity']:.3f}")
        with col2:
            st.metric("Coherence (npmi)", f"{data_lda['model_metrics']['coherence_npmi']:.3f}")
            st.metric("Topic Diversity", f"{data_lda['model_metrics']['topic_diversity']:.3f}")
        ...
        all_topics = sorted(data_lda["topics"].items(), key=lambda x: int(x[0]))
        topics_per_page = 10
        ...
        if "videos_without_topics" in data_lda and data_lda["videos_without_topics"]:
            unassigned_videos = data_lda["videos_without_topics"]
            ...
            for video_id in paged_video_ids:
                title = data_lda["videos"].get(video_id, {}).get("title", "Unknown Title")
    ...
    # ---------------------------------
        ...
        selected_topic = st.selectbox("Select Topic", options=list(data_lda["topics"].keys()))
        topic_info = data_lda["topics"][selected_topic]
        ...
        with tab1:
            ...
            for vid in paged_videos:
                title = data_lda["videos"][vid]["title"]
        ...
        with tab2:
            if "topic_to_subtopics" in data_lda and selected_topic in data_lda["topic_to_subtopics"]:
                st.subheader("Subtopics for this Topic")
                
                for subtopic in data_lda["topic_to_subtopics"][selected_topic]:
                    category = data_lda["subtopic_categories"].get(subtopic, "Unknown")
                    
                    with st.expander(f"{subtopic} ({category})"):
                        # Show subtopic details
                        if "subtopic_keywords" in data_lda and subtopic in data_lda["subtopic_keywords"]:
                            st.write("**Keywords:**")
                            for topic_id, keywords in data_lda["subtopic_keywords"][subtopic].items():
                                if topic_id == selected_topic:
                                    st.write(f"- {', '.join(keywords)}")
                        
                        # Show videos in this subtopic
                        if "subtopic_to_videos" in data_lda and subtopic in data_lda["subtopic_to_videos"]:
                            subtopic_videos = data_lda["subtopic_to_videos"][subtopic]
    ...
    # ---------------------------------
        ...
        video_ids = list(data_lda["videos"].keys())
        video_titles = [data_lda["videos"][vid]["title"] for vid in video_ids]
        ...
        video_data = data_lda["videos"][selected_video]
        ...
        for ch_id, ch_info in data_lda.get("channel_summary", {}).items():
        ...
        if "video_to_subtopics" in data_lda and selected_video in data_lda["video_to_subtopics"]:
            st.write("**Assigned Subtopics:**")
            for subtopic in data_lda["video_to_subtopics"][selected_video]:
                category = data_lda["subtopic_categories"].get(subtopic, "Unknown")
        ...
        with prob_tab1:
            ...
            for topic_id, prob in sorted_topics:
                topic_words = ", ".join(data_lda["topics"].get(topic_id, {}).get("top_words", ["N/A"]))
                all_topics.append({
                    "Topic": topic_id,
                    "Probability": prob,
        ...
        with prob_tab2:
            ...
            if not video_data["topics"]:
                ...
                for topic in video_data["topics"]:
                    ...
                    topic_words = ", ".join(data_lda["topics"].get(topic_str, {}).get("top_words", ["N/A"]))
        ...
        with prob_tab3:
            ...
            if "subtopic_assignments" in data_lda:
                assigned_subtopics = []
                
                # Iterate through all topic groups
                for group_str, group_data in data_lda["subtopic_assignments"].items():
    ...
    # ---------------------------------
        ...
        if "merged_topic_groups" not in data_lda:
            st.warning("Topic group data not available")
            st.stop()
        
        topic_groups = list(data_lda["merged_topic_groups"].keys())
        selected_group = st.selectbox("Select Topic Group", options=sorted(topic_groups, key=int))
        group_data = data_lda["merged_topic_groups"][selected_group]
    ...
    # Display group overview
        ...
        if "subtopic_assignments" not in data_lda or selected_group not in data_lda["subtopic_assignments"]:
            st.warning("No subtopic assignments for this group")
        else:
            group_assignments = data_lda["subtopic_assignments"][selected_group]
        ...
        if (
            "subtopic_assignments" in data_lda
            and "unassigned_videos_by_group" in data_lda["subtopic_assignments"]
            and str(selected_group) in data_lda["subtopic_assignments"]["unassigned_videos_by_group"]
        ):
            unassigned = data_lda["subtopic_assignments"]["unassigned_videos_by_group"][str(selected_group)]
        ...
        if "channel_summary" not in data_lda or not data_lda["channel_summary"]:
            st.warning("No channel summary data found.")
            st.stop()

        channel_summary = data_lda["channel_summary"]
        all_video_ids = list(data_lda["videos"].keys())

        # Create a dict for quick title lookup
        video_titles = {vid: data_lda["videos"][vid]["title"] for vid in all_video_ids}
                            ...
                            for subtopic in sorted(group_map[group_index][category]):
                                ...
                                if st.session_state.get(subtopic_key, False):
                                    ...
                                    for i, vid in enumerate(paginated_vids):
                                        title = data_lda["videos"].get(vid, {}).get("title", "Unknown Title")
